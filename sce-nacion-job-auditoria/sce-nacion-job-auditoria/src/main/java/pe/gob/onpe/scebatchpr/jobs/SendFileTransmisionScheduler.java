...
@Component
public class SendFileTransmisionScheduler {

	private static final Logger LOG = LoggerFactory.getLogger(SendFileTransmisionScheduler.class);
	...
	public SendFileTransmisionScheduler(
	...
	public void scheduleTaskWithCronExpressionFile() {
		LOG.info("Method executed. Current time is file = {}", new Date());
		
		if (!isRunning.compareAndSet(false, true)) {
            LOG.warn("Job anterior aun en ejecucion de transmision de archivos, saltando esta iteración");
            return;
        }
		...
		try { 
			...
			if(envioMq) {
				LOG.info("Se realiza el envio de archivos por MQ");
				this.transmisionMqService.enviarArchivos();
			}
		} catch (IOException e) {
