...
public class FirmaDocumentoInstalacionSufragioServiceImpl implements FirmaDocumentoInstalacionSufragioService {
	...
	private void firmarDocumentoInstalacionSufragio(VwArchivoInstalacionSufragioSinFirmarDto vs, String pathBase) {
		...
		try (InputStream firmadoStream = firmaDigitalDocService.firmarArchivo(rutaArchivo)) {
			...
			Archivo archivoFirmado = Archivo.builder().guid(guid).nombre(nombreArchivo).nombreOriginal(nombreArchivo)
					.formato(extension).peso(String.valueOf(Files.size(Paths.get(nuevaRutaArchivo))))
					.ruta(nuevaRutaArchivo).activo(SceConstantes.ACTIVO).fechaCreacion(new Date())
					.usuarioCreacion(Constantes.USUARIO_JOB).build();
	...
	public void firmarDocumentos() {
		List<VwArchivoInstalacionSufragioSinFirmarDto> vss = this.archivoTransmisionRepositoryCustom.listarArchivosInstalacionSufragioSinFirmar();
		if (vss != null && !vss.isEmpty()) {
			for(VwArchivoInstalacionSufragioSinFirmarDto vs:vss){
				this.firmarDocumentoInstalacionSufragio(vs, ubicacionFile);
			}
		}
	}
